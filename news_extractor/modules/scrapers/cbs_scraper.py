...
class CBSScraper(BaseScraper):
    def __init__(self):
        super().__init__()
        self.url = 'https://www.cbsnews.com/feature/election-2024/'
        self.outlet = 'cbs'
    ...
    def scrape(self):
        ...
        article_cards = soup.find_all('article', class_='item--topic-politics')
        more_articles = soup.find('section', {'id': 'component-election-2024'}).find_all('article', class_='item--topic-')
        article_cards.extend(more_articles)
        articles_set = set()
        for card in article_cards:
            href = card.find('a').get('href')
            if href and 'video' not in href and 'bit.ly' not in href:
